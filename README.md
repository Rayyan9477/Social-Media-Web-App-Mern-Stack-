...
2.  **Environment Variables:**
    -   Modify the `JWT_KEY` environment variable within the `docker-compose.yml` file under the `backend` service with your own secret key.
3.  **Build and Run:**
    ```bash
...
5.  **To stop the application:**
    ...
    docker-compose down
    ```
6.  **To stop and remove volumes (including database data):**
    ```bash
